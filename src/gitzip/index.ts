...
import { removeTailSlash, REPO_EXP, resolveUrl, GITHUB_PROVIDED_URL, GITHUB_DOWNLOAD_URL, request } from './utils'
            ...
            if(matches[4]){ // case: tree
                ...
                return new Promise(function(resolve, reject){
                    ...
                    .then(function(res){
                        ...
                        results.path = removeTailSlash(res.path);
...
function callAjax(url: string, params: object, type?: "arraybuffer" | "blob" | "json"){
  return request({
    url,
    query: params,
    responseType: type
...
var _checkAndGetDownloadURL = function(url){
    if ( url ) {
        if ( GITHUB_DOWNLOAD_URL.test(url) ) return Promise.resolve(url);
...
function downloadZip(url, callbackScope){
    ...
    return _checkAndGetDownloadURL(url)
        .then(function(validUrl){
            return callAjax(validUrl, params, "text")
        })
        .then(function(xmlResponse){
...
function zipIt(zipName, url, callbackScope){
    callbackScope = callbackScope || _global;
    if(url && GITHUB_PROVIDED_URL.test(url)){
    ...
    .then(function(resolved){
        if(!resolved.path){
            ...
            if(resolved.type == "tree"){
                // for tree handles
                callAjax("https://api.github.com/repos/" + resolved.author + 
